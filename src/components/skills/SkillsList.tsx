...
const SkillsList: React.FC<SkillsListProps> = ({ isManager }) => {
  ...
  const handleAdd = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!name.trim() || mark <= 0) {
      setError("נא למלא שם וציונים חוקיים");
      return;
    }
    setError(null);
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!editingSkill || !name.trim() || mark <= 0) {
      setError("יש למלא שם וציונים חוקיים");
      return;
    }
    setError(null);
    const token = localStorage.getItem("token");
    const res = await fetch(`${API_URL}/${editingSkill.skillsId}`, {
      method: "PUT",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${token}`,
      },
      body: JSON.stringify({ skillsId: editingSkill.skillsId, name, mark }),
    });
    ...
    if (res.ok) {
      ...
      setError(`שגיאה בעדכון: ${errorText}`);
    }
  };
  ...
  return (
    <div dir="rtl" style={{ minHeight: "100vh", background: "#4978c9", padding: "0" }}>
      <div style={{
        ...
        {isManager && (
          <form
            ...
            style={{
              ...
              background: "#ffd000", borderRadius: 8, padding: 16,
          ...
          >
            <input
              ...
              style={{
                ...
                border: "1px solid #bbb", fontSize: 15, minWidth: 120
              }}
            />
            <input
              type="number"
            ...
            <button type="submit" style={{
              ...
              fontWeight: "bold", fontSize: 15, minWidth: 80
            }}>
              {editingSkill ? "עדכן" : "הוסף"}
            </button>
            {editingSkill && (
        ...
        <h2 style={{ textAlign: "center", color: "#ffd000", margin: "24px 0 16px", fontWeight: "bold" }}>
          רשימת כישורים
        </h2>
        <ul style={{
          ...
          {skills.map((s) => (
            <li key={s.skillsId} style={{
              background: "#fff", borderRadius: 12, padding: "18px 20px",
              minWidth: 260, maxWidth: 340, boxShadow: "0 1px 3px #0002",
              display: "flex", flexDirection: "column", alignItems: "flex-start"
            }}>
              <div style={{ fontWeight: "bold", fontSize: 18 }}>{s.name}</div>
              <div style={{ color: "#555" }}>ציון: {s.mark}</div>
              {isManager && (
                <div style={{ marginTop: 10 }}>
                  <button
                    onClick={() => startEdit(s)}
                    style={{
                      ...
                      borderRadius: 6, padding: "4px 10px", marginLeft: 6,
                      cursor: "pointer", fontWeight: "bold"
                    }}>
                    עדכן
                  </button>
                  <button
                    onClick={() => handleDelete(s.skillsId)}
                    style={{
